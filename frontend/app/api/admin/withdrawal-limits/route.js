...
export async function GET(request) {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/admin/withdrawal-limits`, {
      method: 'GET',
      headers: {
    ...
    console.error('Admin withdrawal limits GET API error:', error);
...
export async function PUT(request) {
  try {
    ...
    console.error('Admin withdrawal limits PUT API error:', error);
    return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
  }
}
