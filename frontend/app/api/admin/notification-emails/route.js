...
export async function GET(request) {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/admin/notification-emails`, {
      method: 'GET',
      headers: {
    ...
    console.error('Admin notification emails GET API error:', error);
...
export async function PUT(request) {
  try {
    ...
    console.error('Admin notification emails PUT API error:', error);
    return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
  }
}
