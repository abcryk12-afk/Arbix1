...
export async function GET(request) {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/admin/investment-packages`, {
      method: 'GET',
      headers: {
    ...
    console.error('Admin investment packages GET API error:', error);
...
export async function PUT(request) {
  try {
    ...
    console.error('Admin investment packages PUT API error:', error);
    return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
  }
}
