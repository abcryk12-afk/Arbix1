...
export async function GET(request) {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/admin/footer-stats-overrides`, {
      method: 'GET',
      headers: {
    ...
    console.error('Admin footer stats overrides GET API error:', error);
...
export async function PUT(request) {
  try {
    ...
    console.error('Admin footer stats overrides PUT API error:', error);
    return NextResponse.json({ success: false, message: 'Internal server error' }, { status: 500 });
  }
}
